...
> 生成时间:2026-08-16,当前版本 **v0.25.1**(选人/选图/暂停统一精修 + 地形脚步音效)。
...
- v0.25.0 已合入：选人/选图界面重做——Kenney 双线雕花重框(96px,panel-double-gold/soft/clear)+ corners-gold 四角饰;选人卡立绘区/铭牌/武器行+交错入场;选图卡地形预览+难度星;余烬粒子层(新层 `css/select-v25.css`);清除锁定卡 🔒。
- v0.24.0 已合入：去 AI 味专项——Kenney Fantasy UI Borders(CC0)按亮度重映射为项目金色阶后统一全部二级界面框体(`assets/ui/kenney/`,新层 `css/unify-v24.css` 管框体+色板 token+动效);emoji 图标全部替换(图鉴 tab/成就/暂停/索敌/Boss/警告/favicon→图集图标或 CSS 绘制);界面切换/升级卡交错入场/toast 滑入动效;新增 `test/allui-shots.js` 全界面审查工具。
...
- v0.23.3 已合入：左下角金币/击杀内容左对齐，图标贴左边框，数字增长也不会越界。
- v0.23.4 已合入：金币/击杀框统一贴图对齐；挥剑/挥斧换免费 MP3；新增 Kenney CC0 跑步脚步音效。
- v0.25.1 已合入：草地/道路脚步区分；暂停面板双线雕花金框；选人/选图/暂停按钮统一主菜单金属风格。
