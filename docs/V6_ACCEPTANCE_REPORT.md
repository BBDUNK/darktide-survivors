...
- 六角色 72 个四方向虚影动作、击杀成长、躯干锚点由 `art-probe.js` 继续绿灯（锚点全部 [32,54]，帧数/帧率/左≠右全部通过）。
- 贤者光弹折射/烙印/四目标光束、圣女贴地光环、寒霜冲击 12 帧 160×160、特斯拉塔/电圈/战车电磁炮、火瓶碎裂与地面火池、逍遥御剑、所罗门秘典与召唤恶魔等 V5 素材继续通过 `art-probe.js`（帧数/尺寸断言）与 `combat-vfx-probe.js`（塔身/斩击角度/箭矢像素）实测；无新失败项，未返工。
- `test/shots.js` 修复主菜单百科全书按钮的 emoji 选择器后全流程截图通过（15 张，无控制台错误）。
- 四分辨率截图复跑通过：1280×720 / 960×540 / 390×844 / 844×390，标题、主菜单、选角色、选地图、HUD、暂停全绿（`shots/v6-baseline/`）。
- **390px 手机主菜单修复**：原横排“圣坛 190px + 按钮 270px + 公告板 190px”在 390px 宽度互相挤压；`feature-v21.css` 新增 `@media (max-width:520px)`，主菜单改纵向堆叠、隐藏公告板、压缩圣坛与标题字号，所有按钮保持 ≥44px 点击区，无遮挡/裁切。
- 主菜单强化圣坛、战报、角色/地图选择、联机大厅、成就、百科、设置、升级、开箱、暂停、结算、无尽入口与训练场均有本次截图覆盖；暗金巴洛克九宫格组件继续复用，未出现整图拉伸。
...
- **Boss 动画修复**：一次性动作（变招/死亡/变身/受击）不再叠加每个敌人随机的 `animo` 相位取模，避免 8 帧动画跳回中间残缺帧；循环动画保留 animo 错相。视觉探针 `v6-slimeking/bonelord-probe` 重跑全绿。
- **Boss 血量**：骸骨领主 45000、深渊之眼 100000、暗潮魔王 225000（均砍半）；暗潮 P2 真身 500 万保留。`headless.js` R7 同步更新阈值。
- **训练场面板可收放**：新增“收起 ▸ / 展开 ◂”把手；收起后仅保留小按钮，不再遮挡小地图；新增“角色”下拉切换六角色（`applyTestAction type:'char'`）。
- **训练场键盘修复**：面板交互后清空 `Engine.keys`；点回画布时 blur 面板控件，消除 WASD 失灵与自动行走。
- **骑士终极**：誓约圣剑进化后周期性天降黄金巨剑追击（审判期间频率/数量翻倍），并保留誓约盾/审判加成。
- **法师终极**：贤者光弹光束索敌范围 540→1080；持续闪电锁敌，锁定越久颜色越深、光柱越粗。
- 新增 `test/v6-training-panel-probe.js`；回归测试全绿。

## 用户反馈修复轮 2（v0.21.2：选人/图鉴/训练场 UI）

- 选人页只保留最外层金框；卡片网格、角色卡、信息框、底部按钮的金框全部移除或改深色细线。
- 百科全书内容区 1200→820px，卡片最小宽 360→260px，整体缩小不再铺满全屏。
- 训练场“展开/收起”把手与面板按钮改为扁平深色细边，去掉不适配的厚金框。
- `shots.js` 全流程截图重跑无控制台错误。
