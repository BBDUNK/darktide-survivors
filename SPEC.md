...
v0.19 精修覆盖 143 个运行时命名、336 帧:六名 `char_*` 均有四帧待机、奔跑、攻击动作,并覆盖 16 种普通敌人、4 个 Boss、全部弹体/武器/进化/被动/系统图标、掉落物、地图装饰和 10 组精灵特效。动画按逐素材 FPS 播放;动作图集缺失或加载失败时自动回退角色基础程序素材。图集由 `node tools/art/build-atlas.js` 构建并通过 `quality-report.json` 门禁;开放素材映射与固定来源版本见 `tools/art/open-assets-manifest.json`。
