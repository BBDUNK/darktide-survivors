...
- Source: project-authored procedural pixel art generated by `tools/art/generate-gothic-menu-frame.py`; not a third-party download.
- Output: `assets/ui/gothic-frame.png` (96x96 RGBA, 24px nine-slice border, dark gothic metal with antique-gold baroque ornament).
- Integration: applied to `.menu-screen` with `border-image` in `css/style.css`.
...
- Used file: `assets/audio/music/battle-theme-a.mp3`, played via WebAudio for every boss battle
  (`boss_slime` / `boss_bone` / `boss_abyss` / `boss_dark`). Author's own game/soundtrack is 100% free (pixelsphere.org).
