...
for (const enemy of enemies) {
  const source = enemy === 'skeleton'
    ? 'assets/art-v5/sprites/elites/elite_skeleton_actions_repaired.png'
    : `assets/art-v3/sprites/elites/elite_${enemy}_actions.png`;
  for (const [suffix, row, fps] of enemyActions) {
    add({
...
for (const boss of bosses) {
  // The original slime-king master is a 7x4 grid.  Its previous 8x5 crop
  // crossed cell boundaries and produced half-monsters.  V5 keeps the same
  // bone-crowned green design but uses the correctly rebuilt action sheet.
  const source = boss === 'boss_slimeking'
    ? 'assets/art-v5/sprites/bosses/boss_slimeking_actions_repaired.png'
    : `assets/art-v4/repaired/bosses/${boss}_actions.png`;
...
// V5 vertical-slice projectiles and circular ground effects.  These keep their
// authored square footprint; runtime animation advances frames without a
// second scale animation, eliminating the former flattened/jumping nova.
add({ name: 'p_arcane_orb', source: 'assets/art-v5/sprites/vfx/arcane_orb_actions.png',
  frameSize: [48, 48], frameRow: 0, frames: 8, fps: 14,
  size: [32, 32], anchor: [24, 24], renderScale: 1, maxBaselineDrift: 48 });
add({ name: 'vfx_frost_kiss_radial', source: 'assets/art-v5/sprites/vfx/frost_kiss_radial_actions_linear.png',
  frameSize: [160, 160], frameRow: 0, frames: 12, fps: 14,
  size: [160, 160], anchor: [80, 80], renderScale: 1, maxBaselineDrift: 160 });
add({ name: 'p_tesla_cannon', source: 'assets/art-v5/sprites/weapons/tesla_cannon_projectile.png',
  frameSize: [96, 96], frameRow: 0, frames: 8, fps: 15,
  size: [80, 80], anchor: [48, 48], renderScale: 1, maxBaselineDrift: 96 });
add({ name: 'vfx_tesla_cannon_impact', source: 'assets/art-v5/sprites/vfx/tesla_cannon_impact.png',
  frameSize: [96, 96], frameRow: 0, frames: 10, fps: 18,
  size: [96, 96], anchor: [48, 48], renderScale: 1, maxBaselineDrift: 96 });
add({ name: 'vfx_tesla_overload', source: 'assets/art-v5/sprites/vfx/tesla_overload_ground.png',
  frameSize: [160, 160], frameRow: 0, frames: 12, fps: 18,
  size: [160, 160], anchor: [80, 80], renderScale: 1, maxBaselineDrift: 160 });
...
// Full four-direction tank action system.  Row order is generated by
// build-v5-vertical-slice.py as direction x (idle, walk, attack).
['down', 'up', 'left', 'right'].forEach((direction, dirIndex) => {
  [['idle', 0, 7], ['walk', 1, 11], ['attack', 2, 14]].forEach(([action, actionIndex, fps]) => {
    add({ name: `tesla_battle_tank_${action}_${direction}`,
      source: 'assets/art-v5/sprites/weapons/tesla_battle_tank_full_actions.png',
      frameSize: [96, 96], frameRow: dirIndex * 3 + actionIndex,
      frames: 8, fps, size: [96, 96], anchor: [48, 91], renderScale: 1,
      maxCentroidDrift: 4, maxBaselineDrift: 2 });
  });
});
add({ name: 'tesla_battle_tank', source: 'assets/art-v5/sprites/weapons/tesla_battle_tank_full_actions.png',
  frameSize: [96, 96], frameRow: 0, frames: 8, fps: 7,
  size: [96, 96], anchor: [48, 91], renderScale: 1,
  maxCentroidDrift: 4, maxBaselineDrift: 2 });
